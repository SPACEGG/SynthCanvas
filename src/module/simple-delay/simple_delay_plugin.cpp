...
namespace synth_canvas::simple_delay_plugin
{
    ...
    const clap_plugin_descriptor *SimpleDelayPlugin::descriptor()
    {
        return &desc;
    }
    ...
    bool SimpleDelayPlugin::activate(double sampleRate, uint32_t minFramesCount, uint32_t maxFramesCount) noexcept
    {
        if (!Plugin::activate(sampleRate, minFramesCount, maxFramesCount))
            return false;
    ...
    bool SimpleDelayPlugin::audioPortsInfo(uint32_t index, bool isInput, clap_audio_port_info *info) const noexcept
    {
        if (index > 0)
            return false;
    ...
    uint32_t SimpleDelayPlugin::paramsCount() const noexcept
    {
        return PARAM_COUNT;
    }
    ...
    bool SimpleDelayPlugin::paramsValueToText(clap_id paramId, double value, char *display, uint32_t size) noexcept
    {
    ...
    bool SimpleDelayPlugin::paramsTextToValue(clap_id paramId, const char *display, double *value) noexcept
    {
        ...
        switch (paramId)
        {
            ...
            case PARAM_MIX:
                ...
                if (unit_str == "%") {
                    *value = std::clamp(parsedValue / 100.0, 0.0, (paramId == PARAM_FEEDBACK ? 0.95 : 1.0));
    ...
    void SimpleDelayPlugin::paramsFlush(const clap_input_events *in, const clap_output_events *out) noexcept
    {
        if (!in) return;
    ...
    clap_process_status SimpleDelayPlugin::process(const clap_process *process) noexcept
    {
        ...
        if (in_count > 0)
        {
            ...
            num_channels = std::min({num_channels, process->audio_inputs[0].channel_count, (uint32_t)_delayBuffer.size()});
        }


        // Calculate read head for delay. Clamp to avoid reading beyond allocated buffer which can happen if delay_samples is too large
        uint32_t delay_samples = secondsToSamples(_delayTime);
        delay_samples = std::min(delay_samples, _bufferSize -1); // Ensure it doesn't exceed buffer boundary. Minimal delay is 1 sample.
