...
static uint32_t clap_get_plugin_count(const struct clap_plugin_factory *factory) { return 1; }

static const clap_plugin_descriptor_t *clap_get_plugin_descriptor(
...
static const clap_plugin_t *clap_create_plugin(const struct clap_plugin_factory *factory,
                                               const clap_host_t *host, const char *plugin_id) {
    if (strcmp(plugin_id, s_plugin_descriptor->id) == 0) {
        auto *plugin = new clap::SinePlugin(host);
...
static bool clap_init(const char *plugin_path) {
    s_plugin_descriptor = clap::SinePlugin::descriptor();
    return true;
}
