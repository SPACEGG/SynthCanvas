...
static auto clapCreatePlugin(const struct clap_plugin_factory* factory, const clap_host_t* host,
    ...
    if (std::strcmp(plugin_id, s_plugin_descriptor->id) == 0) {
        auto* plugin = new OscillatorPlugin(plugin_id, host);
...
static auto clapInit(const char* plugin_path) -> bool {
    s_plugin_descriptor = OscillatorPlugin::descriptor();
    return true;
}
...
extern "C" CLAP_EXPORT const clap_plugin_entry_t clap_entry = {
    ...
    .init = synth_canvas::oscillator_plugin::clapInit,
    .deinit = synth_canvas::oscillator_plugin::clapDeinit,
    .get_factory = synth_canvas::oscillator_plugin::clapGetFactory,
};
