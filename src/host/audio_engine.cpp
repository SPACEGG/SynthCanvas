...
void AudioEngine::processSinglePlugin(PluginHost* host, int32_t num_frames) {
    ...
    for (const auto& conn : _current_render_state->connections) {
        if (conn.to_node == node_id && conn.type == ConnectionType::kAudio) {
...
void AudioEngine::routePluginOutputs(PluginHost* host) {
    ...
    // Dequeue all events generated by the plugin during this block
    while (output_queue.try_dequeue(ev)) {
        uint32_t source_node_id = host->getInstanceId();

        // Check connections to see where this event should go
        for (const auto& conn : _current_render_state->connections) {
            if (conn.from_node == source_node_id && conn.type == ConnectionType::kEvent) {
                // Find the target plugin host
                // Since _current_render_state only has sorted_modules (vector), we might need a map
                // for fast lookup? However, linear search in sorted_modules is acceptable for small
                // graph sizes, OR we can rely on ModuleRouter to provide direct pointer connections
                // in the future. For now, let's search in sorted_modules.
                for (PluginHost* target_host : _current_render_state->sorted_modules) {
                    if (target_host->getInstanceId() == conn.to_node) {
