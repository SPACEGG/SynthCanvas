...
#if !defined(_WIN32)
// --- Full implementation for Android/Linux ---
...
public:
    ...
    std::function<void(uint32_t, uint32_t)> on_gui_request_resize;
...
private:
    ...
    struct AppToEngineParamQueueValue { void *cookie; double value; };
    struct EngineToAppParamQueueValue { void update(const EngineToAppParamQueueValue& v) noexcept { if (v.has_value) { has_value = true; value = v.value; } if (v.has_gesture) { has_gesture = true; is_begin = v.is_begin; } } bool has_value = false; bool has_gesture = false; bool is_begin = false; double value = 0; };
    ...
    enum PluginState { Inactive, InactiveWithError, ActiveAndSleeping, ActiveAndProcessing, ActiveWithError, ActiveAndReadyToDeactivate, };
...
// A dummy class that provides the same API as the real PluginHost,
// but does nothing. This is to allow compilation on Windows for the
// Godot editor, where audio functionality is not needed.
class PluginHost {
public:
    PluginHost() {}
    ~PluginHost() {}
    bool load(const std::string &path, int pluginIndex) { return false; }
    void unload() {}
    bool canActivate() const { return false; }
    void activate(int32_t, int32_t) {}
    void deactivate() {}
